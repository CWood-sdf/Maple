...
void AST::prepareInterpreter(std::string f) {
	...
	for (auto& opString : operators) {
		auto op = opString.getReference();
	...
	for (auto& opString : unaryOperators) {
		auto op = opString.getReference();
		// If the operator is not letter based, then add the first character to
		// the set
		if ((op[0] >= 'a' && op[0] <= 'z') || (op[0] >= 'A' && op[0] <= 'Z')) {
...
AST::Type AST::getNextToken() {
	...
	// Handle string literals
		...
		while (file[i] != '"') {
			...
			if (file[i] == '\n') {
				throwError(
					"Unclosed string literal (newlines can't be in "
					"strings)",
					getLine());
				currentLine++;
			}
	...
	// Handle character literals
		...
		while (file[i] != '\'') {
			if (file[i] == '\n') {
				throwError(
					"Unclosed character literal (newlines can't be in "
					"character "
					"literals)",
					getLine());
				currentLine++;
